    ...
    def color_update(self, col, nam_block, nam_item):
        ...
        for other in (
            ...
            if p2[nam_item] != parent[nam_item]:
                ...
                print("****************Not equal1")
                self.tree[col_alt].model().setData(
                    newIndex, color_diff, Qt.ForegroundRole
                )
                # TODO: insert arrow
            else:
                ...
                print("****************equal1")
                self.tree[col_alt].model().setData(
                    newIndex, color_chng, Qt.ForegroundRole
                )
                # TODO: remove arrow
                ...
                if re.search(pattern, line):  # replace as NAMELIST only occurs once
                    ...
                    if "&" in line:
                        ...
                        while ln[-1] == ",":
                            nxt = [
                                x
                                for j, x in enumerate(open(fname))
                                if j
                                in [
                                    count,
                                ]
        ...
        for i, line in enumerate(open(namelist_in)):
            if len(line.strip()) > 0:
                ...
                if nl_dict.get(nam_name) is not None and nam_item is not None:
                    nl_dict[nam_name][
                        nam_item
                    ] = nam_val  # remember we may be adding new nam_vals here
